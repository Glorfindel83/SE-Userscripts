...
function setup(question, callback) {
  var reviewItemID = null;
  if (isModerator && isMetaSE) {
    // Find Close Votes review item
    let questionID = question.attr('data-questionid');
    $.get("https://" + location.host + "/posts/" + questionID + "/timeline", function(data) {
      let reviewItems = $(data).find("a[href^='/review/close/']");
      if (reviewItems.length > 0) {
        let row = reviewItems.parents("tr");
        let nextRow = row.next();
        if (row.attr('data-eventtype') != nextRow.attr('data-eventtype') ||
            row.attr('data-eventid') != nextRow.attr('data-eventid')) {
          // Review item not completed yet
          reviewItemID = row.attr('data-eventid');
        }
      }
      createDialog(question, reviewItemID);
      callback(question, reviewItemID);
    });
  } else {
    createDialog(question, reviewItemID);
    callback(question, reviewItemID);
...
function main(question, summary) {  
  ...
  if (question == null) {
    ...
    menu.append(cell);    
    setup(question, function(question, reviewItemID) {
      button.click(function() {
        buttonClicked(question, reviewItemID);
      });
    });
  }
}

function buttonClicked(question, reviewItemID) {
  if (typeof(reviewItemID) == 'undefined') {
    reviewItemID = null;
  }
...
<aside class="s-modal bg-transparent pe-none js-stacks-managed-popup js-fades-with-aria-hidden" id="modal-base" tabindex="-1" role="dialog" aria-labelledby="mod-modal-title" aria-describedby="mod-modal-description" aria-hidden="false">    
  ...
  html += getHTMLForOption('review', 'Complete the Close Votes review queue item');
  html += `
        </div>
        <br/>
...
function createDialog(question, reviewItemID) {  
  // Analyze comments
  let comments = question.find('ul.comments-list');
  ...
  saviourOfLostSouls.submitDialog = function() {
    ...
    if (selected("comment")) {
      ...
      let comment = window.location.host === "softwarerecs.stackexchange.com"
       ...
       ? ("Hi " + author + ", welcome to MathOverflow! " +
          ...
          "Your question is definitely [off-topic](/help/on-topic) and better deleted here.")
       : ("Hi " + author + ", welcome to the Stack " + (window.location.host === "meta.stackexchange.com" ? "Exchange Network" : "Overflow") + " Meta site! " +
    ...
    if (selected("review")) {
      window.setTimeout(function() {
        ...
        $.post("https://" + location.host + "/review/task-reviewed/" + reviewItemID, {
          taskTypeId: 2,
          taskResultTypeId: 6,
          fkey: fkey
        }, function(data) {
      ...
      }, 250); // small delay to make sure the close vote is registered
    }
